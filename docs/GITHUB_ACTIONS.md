...
PR context appears in text, Markdown, JSON, and AI-ready output. It is context only: rules, scores, review decisions, and failure thresholds remain diff-authoritative. On events without pull request metadata, the Action runs without PR context.
...
For a low-friction, opt-in pull-request setup, copy [the automatic prior-report workflow](examples/github-actions-automatic-pr-comparison.yml). It retains each report/manifest pair as a short-lived artifact and, on a later synchronize event, selects evidence only from the pull request's exact previous commit. It accepts a prior report even when that run failed solely because the configured risk threshold was reached. When the previous artifact is missing or expired, it publishes the current report without a comparison and emits a notice. The selection policy is visible in repository-owned YAML.
