...
For a low-friction, opt-in pull-request setup, copy [the automatic prior-report workflow](examples/github-actions-automatic-pr-comparison.yml). It retains each report/manifest pair as a short-lived artifact and, on a later push to the same pull-request branch, selects the latest successful earlier run and supplies its evidence to Merge Guard. The workflow makes that artifact-selection policy visible in repository-owned YAML; it deliberately falls back to an ordinary non-comparison report when no earlier successful run exists.
