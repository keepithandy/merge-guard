...
Repositories that want a turnkey comparison experience can adopt the caller-owned [automatic prior-report workflow](examples/github-actions-automatic-pr-comparison.yml). Its selection policy is intentionally narrow and auditable: it looks only at successful, completed `pull_request` runs of the named workflow on the same head branch, excludes the current head SHA, and chooses the most recent match returned by GitHub. The policy and artifact retention remain under repository control; a first push or a missing prior artifact produces a normal report without comparison rather than an inferred clean result.
