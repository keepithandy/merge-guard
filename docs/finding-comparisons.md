...
Repositories that want a turnkey comparison experience can adopt the caller-owned [automatic prior-report workflow](examples/github-actions-automatic-pr-comparison.yml). Its selection policy is intentionally narrow and auditable: on a `synchronize` event, it looks only for a `pull_request` run of the named workflow whose pull request number and head SHA match the event's exact previous commit. This supports reruns and force-pushes without selecting evidence from another pull request or an arbitrary older branch commit. A first push, missing artifact, expired artifact, or unavailable prior run produces a normal current report without comparison rather than an inferred clean result. The workflow retains report evidence even when the scan crosses its configured failure threshold.
...
`npm run test:evidence-reproducibility` additionally locks comparison JSON and Markdown bytes across isolated local, CI-style, repeat, Node, and operating-system lanes.
