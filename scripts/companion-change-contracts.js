...
assert.deepEqual(evaluateCompanionChanges([
    'diff --git a/src/save/load.js b/src/save/load.js',
    'diff --git a/src/save/load.js b/src/save/load.js',
    'diff --git a/migrations/001-save.sql b/migrations/001-save.sql',
    'diff --git a/test/save.test.js b/test/save.test.js'
  ].join('\n'), contracts), [{ id: 'save-migration', status: 'satisfied', triggeredBy: ['src/save/load.js'], missing: [] }]);
assert.deepEqual(evaluateCompanionChanges(diff, [null, {}, { id: 'incomplete', trigger: 'src' }]), []);
